# Utilities
```
This is an adaption from the `DecisionTree.jl`-package
```
...
function build_forest(
    ...
    n_features = size(features, 2)
    if impurity_importance && n_features != DecisionTree_modified.n_features(model)
...
function _build_forest(
    ...
    normalized_importance = if !impurity_importance
        ...
        for tree in forest
            ti = DecisionTree_modified.impurity_importance(tree; normalize=true)
            if !isempty(ti)
                fi .+= ti
            end
        end
